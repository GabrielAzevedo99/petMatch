# 🔧 Força UTF-8 no runtime (corrige problema de acentuação no PS 5.1)
...
$behaviors = @('Calmo','Ativo','Independente','Carinhoso','Brincalhão','Tranquilo','Alegre','Tímido','Protetor','Sociável','Curioso','Quieto')
...
for ($i = 1; $i -le 50; $i++) {
    ...
    $story = "Encontrado em " + (Get-Random -InputObject @(
        'um parque','uma feira','um bairro residencial','uma área rural','um bosque','uma rua tranquila'
    )) + ", foi resgatado com " + (Get-Random -InputObject @(
        'muita fome','um pouco de medo','saúde estável','um ferimento leve','muita energia'
    )) + "."
...
# 🔥 Geração segura do JSON
$json = $animals | ConvertTo-Json -Depth 10

$utf8NoBom = New-Object System.Text.UTF8Encoding($false)
[System.IO.File]::WriteAllText("..\data\animals.json", $json, $utf8NoBom)

Write-Output "Wrote $($animals.Count) animals to data\animals.json"